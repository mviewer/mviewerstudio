var mv = (function () {
    ...
    return {
            ...
            $("#appsListTable").append(
                `<div class="col-md-12">
                    <div class="form-group">
                        <label for="opt-title" i18n="version.comment" class="form-label">${mviewer.tr("version.comment")}</label>
        ...
        changeVersion: (id, version) => {
            fetch(`${ _conf.api }/${id}/version/${version}`, {method: "PUT"})
            .catch(err => console.log(err))
            .then(r => r.json())
                .then(r => {
        ...
        actionVersionFormatter: (value, row, index) => {
            return `
                <a id="versionChangeLink" onclick="mv.changeVersion('${row.id}', '${value}')"><i class="ri-eye-line"></i></a>
                <a id="versionDeleteLink" onclick="mv.deleteVersion('${row.id}', '${value}')"><i class="ri-delete-bin-2-line"></i></a>
            `;
        ...
        appsTableToolbar: () => {
            ...
            const btns = [
                {
                    text: "Créer une version commentée",
                    icon: "bi bi-plus-lg",
            ...
            <div class="toolbar">
                ${ btns.map(b => `
                    <button id="" class="${ b.btnClass }" onclick="${ b.click || '' }">
                        <i class="${ b.icon }"></i>
                        <span>${ b.text }</span>
                    </button>
                `).join("") }
        ...
        getAppsTable: (apps) => {
            ...
            const data = apps.versions.map((v, i) => ({
                number: i, 
                id: apps.config?.id,
                version: v.name,
