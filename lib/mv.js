var mv = (function () {
    ...
    return {
        ...
        showCWSResults: function (results) {
            ...
            var countResults = $("#csw-results").children().length;
            if (countResults > 0) {
                $("#search-message").text("");
                $("#search-message").hide();
            } else {
        ...
        showWMSResults: function (results) {
            ...
            $.each(results, function (index, result) {
                ...
                // applications
                if (result.layerid && result.bbox && result.bbox.length > 0) {
                    var _abstract =
            ...
            var countResults = $("#wms-results").children().length;
            if (countResults > 0) {
                $("#search-message").text("");
                $("#search-message").hide();
            } else {
            ...
            selected_layers.each(function(i,ctl) {
                if (ctl.checked) {
                    ...
                    $(".layers-list-item.active")
                        ...
                        .attr("data-url", mv.escapeXml(conf.url))
                        ...
                        .attr("data-metadata", mv.escapeXml(conf.metadata))
