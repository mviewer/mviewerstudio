var mv = (function () {
  ...
  return {
    ...
    validateFormBaseLayer: () => {
      ...
      mv.addBaseLayer();
      alertCustom(
        "Nouveau fond de plan ajouté avec succès ! Activez-le dans la liste des fonds de plan.",
        "info"
      );
      // Reset input
              ...
              html: `
                                <li class="nav-item">
                                    <a class="nav-link" href="#" data-url="${
                                      ...
                                      app.group
                                    }" onclick="loadApplicationParametersFromRemoteFile(this.attributes[\'data-url\'].value)">Modifier</a>
                                </li>`,
            },
            {
        ...
        .then((r) => {
          if (!r.ok) {
            ...
            fetch(`${_conf.api}/${id}/version`, { method: "POST" })
              ...
              .catch((err) =>
                alertCustom("Erreur lors de la création de l'état.", "danger")
              );
          }
        })
    ...
    actionVersionFormatter: (value, row, index) => {
      ...
      return `
                <a id="versionPreviewLink" onclick="mv.previewVersion('${config.id}', '${
                  row.id
                }')"><span class="custom-tooltip-table"><i class="ri-eye-line"></i><span>Prévisualiser cette version</span></span></a>
                <a id="versionAsNewLink" data-bs-target="#genericModal" data-bs-toggle="modal" onclick="mv.showAlertChangeVersion('${
                  config.id
                }', '${
                  row.id
                }', true)"><span class="custom-tooltip-table"><i class="ri-arrow-go-back-fill"></i><span>Restaurer cette version</span></span></a>
                ${row?.version ? flag : ""}
            `;
    },
    appsTableToolbar: () => {
    ...
    showNamePublishModal: (id, name = "", conflict = false) => {
      ...
      const question = conflict
        ? "Ce nom existe déjà ! <br> Vous pouvez changer le nom ou annuler."
    ...
    publish: (id, name = "", message = "") => {
      ...
      if (!id) {
        return alertCustom(
          "L'ID n'est pas renseigné. Veuillez contacter un administrateur.",
          "danger"
        );
      }
      if (!config.isFile) {
      ...
      fetch(`${url}?${params.toString()}`, {
        ...
        .then((data) => {
          ...
          if (!data.online_file) {
            return alertCustom(
              "Le lien vers le fichier publié n'est pas accessible. Veuillez contacter un administrateur.",
              "danger"
            );
          }
          return data;
