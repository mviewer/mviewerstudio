var mv = (function () {
    ...
    return {
        ...
        validateFormBaseLayer: () => {
            ...
            document.getElementById("BasemapAlertPlaceholder").innerHTML="";

            // Template for alert message
            const alertPlaceholder = document.getElementById('BasemapAlertPlaceholder');
            const alertBasemap = (message, type) => {
            ...
            if (isInvalid){
                alertBasemap('<i class="ri-error-warning-line"></i> Veuillez renseigner les informations manquantes', 'danger')
                return;
            }    
            
            // Add basemap
            mv.addBaseLayer();  
            alertBasemap('<i class="ri-checkbox-circle-line"></i> Nouveau fond de plan ajouté avec succès ! Activez-le dans la liste ci-dessus.', 'info');
            // Reset input      
