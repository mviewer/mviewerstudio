  ...
  return {
          ...
          const navButtons = [
            {
              ...
              html: `
                <li class="nav-item">
                    <a class="nav-link" href="#" data-id="${app.id}" data-url="${app.url}" onclick="window.open('${previewUrl}','mvs_vizualize')">${mviewer.tr("preview")}</a>
              </li>`,
            },
            {
              ...
              html: `
                <li class="nav-item">
                    <a class="nav-link" href="#" data-url="${
                      ...
                      app.group
                    }" onclick="loadApplicationParametersFromRemoteFile(this.attributes[\'data-url\'].value)">${mviewer.tr("modify")}</a>
                </li>`,
            },
            {
              ...
              html: `
                <li class="nav-item">
                    <a class="nav-link" href="#" data-id="${app.id}" data-bs-dismiss="modal" onclick="showAlertDelAppFromList(this.attributes[\'data-id\'].value)">${mviewer.tr("delete")}</a>
                </li>`,
            },
          ];
        ...
        var items = `
                <a href="#" class="list-group-item"
                    ...
                    <div class="row small">
                        ...
                        <div class="col-md-6">${"auth.group.role"}&nbsp;: ${mviewer.tr(_userRole)}</div>
                    </div>
                </a>`;
    ...
    showAlertChangeVersion: (id, version) => {
      ...
      genericModalContent.innerHTML = `
                ...
                <div class="modal-body">
                    <p i18n="modal.restore.explain">${mviewer.tr(modal.restore.explain)}</p>
                    ...
                    <a class="cardsClose save-close zoomCard" data-bs-dismiss="modal" onclick="mv.changeVersion('${id}', '${version}', true)">
                        ...
                        <span i18n="modal.exit.save">${mviewer.tr("modal.restore.app")}</span>
                    </a>
                    <a class="cardsClose notsave-close zoomCard" data-bs-target="#mod-appversions" data-bs-toggle="modal">
                        <i class="bi bi-x-circle"></i>
                        <span i18n="modal.restore.continue">${mviewer.tr("modal.restore.continue")} </span>
                    </a>
        ...
        .then((r) => {
          if (!r.ok) {
            ...
            fetch(`${_conf.api}/${id}/version`, { method: "POST" })
              ...
              .catch((err) =>
                alertCustom(mviewer.tr("alert.create.state"), "danger")
              );
          }
        })
    ...
    actionVersionFormatter: (value, row, index) => {
      ...
      return `
                <a id="versionPreviewLink" onclick="mv.previewVersion('${config.id}', '${
                  row.id
                }')"><span class="custom-tooltip-table"><i class="ri-eye-line"></i><span>${mviewer.tr("release.preview")}</span></span></a>
                <a id="versionAsNewLink" data-bs-target="#genericModal" data-bs-toggle="modal" onclick="mv.showAlertChangeVersion('${
                  config.id
                }', '${
                  row.id
                }', true)"><span class="custom-tooltip-table"><i class="ri-arrow-go-back-fill"></i><span>${mviewer.tr("release.restore")}</span></span></a>
                ${row?.version ? flag : ""}
            `;
    },
    appsTableToolbar: () => {
      ...
      return `
            <div class="toolbar" style="display:inline-flex">
                ...
                <div class="custom-control custom-switch mt-2 ml-2" id="btnSwitchHistoryTagsOnly">
                    ...
                    <label class="custom-control-label" for="inputSwitchHistoryTagsOnly"><span>${mviewer.tr("release.show.tags")}</span></label>
                </div>
            </div>
            `;
    },
                ...
                >
                    <thead>
                        <tr class="paddingTable">
                            ...
                            <th data-field="title" class="titleThema">${mviewer.tr("table.title")}</th>
                            <th data-field="description" class="descriptionThema">${mviewer.tr("table.description")}</th>
                            <th data-field="publisher" class="publisherThema">${mviewer.tr("table.publisher")}</th>
                            <th data-field="identifier" class="urlThema">${mviewer.tr("table.identify")}</th>
                            <th data-field="id" class="idThema">${mviewer.tr("table.id")}</th>
                        </tr>
                    </thead>
                </table>
            `;
    ...
    showNamePublishModal: (id, name = "", conflict = false) => {
      ...
      const question = conflict
        ...
        : "Quel nom souhaitez vous utiliser pour la publication ?";
      genericModalContent.innerHTML = "";
      genericModalContent.innerHTML = `
                <div class="modal-header">
                ...
                <div class="modal-body">
                    ...
                    <!-- input-->
                    <div class="form-group">
                        <label for="relationPublishName">${mviewer.tr("publish.name.new")}</label>
                    ...
                    <a class="returnConf-close" data-bs-target="#genericModal" data-bs-toggle="modal" aria-label="Close"><i class="ri-arrow-left-line"></i> <span i18n="modal.exit.previous">${mviewer.tr("modal.exit.previous")}</span></a>                    
                </div>
            `;
    ...
    showPublishModal: (shareLink = "", iframeLink = "", draftLink = "") => {
      ...
      genericModalContent.innerHTML = `
                <div class="modal-header">
                    <h5 class="modal-title" i18n="modal.publish.title">${mviewer.tr("modal.publish.title")}</h5>
                ...
                <div class="modal-body">
                    ...
                    <!-- input-->
                    <div class="form-group">
                        <label for="publishShareLink">${mviewer.tr("publish.available.share")}</label>
                        <input readonly value="${shareLink}" type="text" class="form-control" id="publishShareLink">
                    </div>
                    <div class="form-group">
                        <label for="publishIframeLink">${mviewer.tr("publish.iframe")}</label>
                    ...
                    <a class="returnConf-close" onclick="mv.refreshOnPublish('${draftLink}')" data-bs-target="#genericModal" data-bs-toggle="modal" aria-label="Close"><i class="ri-arrow-left-line"></i> <span i18n="modal.exit.previous">${mviewer.tr("modal.exit.previous")}</span></a>                    
                </div>
            `;
      publishModal.show();
    },
      ...
      fetch(`${url}?${params.toString()}`, {
        ...
        .then((data) => {
          if (!_conf?.publish_url) {
            return alertCustom(
              mviewer.tr("Configuration manquante pour la publication. Veuillez contacter un administrateur."),
              "danger"
            );
          }
          if (!data.online_file) {
            return alertCustom(
              mviewer.tr("publish.config.missing"),
              "danger"
            );
          }
          return data;
        ...
        .catch((err) => {
          if (err.status == 409) {
            ...
            alertCustom(
              mviewer.tr("publish.alert.error"),
              "danger"
            );
          }
        });
