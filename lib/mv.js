var mv = (function () {
    ...
    return {
        ...
        formatPublishText: (value, row) => {
            let badge = row.isPublish ?
                `<span class="badge badge-publish">${ mviewer.tr("publish") }</span>` :
                `<span class="badge badge-draft">${ mviewer.tr("draft") }</span>`;
            
            let contentText = row.publishedCommit ? mviewer.tr("publish.message") : value;
            return `${ badge } ${ contentText }`;
        },
        isPublished: (compareDate, lastPublishDate) => {
            const lastPublish = moment(lastPublishDate, "YYYY-MM-DD-HH-mm-ss");
            const toCompareDate = moment(compareDate, "YYYY-MM-DD-HH-mm-ss");
            return toCompareDate.isBefore(lastPublish) || toCompareDate.isSame(lastPublish);
        },
        ...
        currentCommitStyle: (row) => {
            return {
                classes: row.publishedCommit && "publishedVersion"
            }
        },
        ...
        getAppsTable: (apps) => {
            ...
            data = data.map((v, i) => {
                ...
                if (lastPublished) {
                    rowsData.isPublish = mv.isPublished(rowsData.date, lastPublished.date);
