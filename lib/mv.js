    ...
    return {
        ...
        showBaseLayers: function (data, classe) {
             ...
             $.each(data, function (index, l) {
                var div = ['<div class="' + classe + ' bl col-sm-6 col-md-3" data-title="'+l.label+'" data-layerid="'+l.id+'" >',
                         '<div class="thumbnail">',
                      '<div class="caption">',
                        '<h4>'+l.label+'</h4>',
                        '<p>'+l.title+'</p>',
        ...
        showWMSResults: function (results) {
            ...
            $.each(results, function (index, result) {
                ...
                if (result.layerid) {
                    var div = [
                        '<div class="ogc-result wms-result col-sm-6 col-md-4" data-title="'+result.title+'" data-layerid="'+result.layerid+'" data-metadata="'+result.metadata+'" data-metadata-csw="'+result['metadata-csw']+'" data-url="'+result.wms+'" data-attribution="'+result.attribution+'" data-type="wms">',
                        '<h3>'+result.title+'</h3>',
                        '<p>'+result.abstract+'</p>',
                        '<input id="'+result.layerid+'-ck" type="checkbox" aria-label="...">',
                        '</div>'].join("");
        ...
        createDubliCore(data) {
            ...
            var dc = ['<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:dc="http://purl.org/dc/elements/1.1/">',
            ...
            $.ajax({
                ...
                success: function(data, status, xhr){
                    ...
                    data.forEach(function (app, index) {
                        ...
                        var item = ['<a href="#" class="list-group-item" data-url="'+_conf.mviewer_instance+app.url+'" onclick="loadApplicationParametersFromHistory(this.attributes[\'data-url\'].value);" >',
