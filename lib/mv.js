    ...
    return {
        ...
        showWMSResults: function (results) {
            ...
            $.each(results, function (index, result) {
                ...
                // - a bbox
                // - a parent layer
                // should be displayed (those with no name should to be used by client applications)
                if (result.layerid && result.bbox && result.bbox.length > 0 && result.parentLayerId) {
                    var _abstract =
                        (result.abstract.length > 200) ? result.abstract.substring(0, 200) + '...' : result.abstract;
                    var div = [];
