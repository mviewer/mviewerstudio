var mv = (function () {
    ...
    return {
        ...
        showStyles: function (styles, layerid, themeid) {
        ...
        showFields: function (fields, layerid, themeid) {
        ...
        getConfLayers: function () {
            ...
            var counter = 0;
            var ogc_type = "";
            var themeid = $('.themes-list-item.active').attr('id');
            selected_layers.each(function(i,ctl) {
                if (ctl.checked) {
                    var conf = $(ctl).closest(".ogc-result").data();
                    if (counter > 0) {
                        addLayer('Nouvelle couche', themeid);
        ...
        showLayerOptions: function (el, themeid) {
            //clear forms
            ...
            if (layer.type === "wms") {
                ...
                ogc.getFieldsFromWMS(layer.url, layerid, themeid);
                ogc.getStylesFromWMS(layer.url, layerid, themeid);
            } else {
        ...
        saveLayerOptions: function () {
            ...
            var layerid = el.attr("data-layerid");
            var themeid = el.parent().parent().attr('id');
            function getLayerbyId(l) {
                return l.id === layerid;
            }
            var layer = config.themes[themeid].layers.find(getLayerbyId);
