var mv = (function () {
    ...
    return {
        ...
        createDublinCore(data) {
            ...
            var org = _userInfo?.groupFullName || "";
        ...
        parseApplication(xml) {
            ...
            newConfiguration({id: app_identifier, isFile: true, date: dateXml});
        ...
        showPublishModal: (shareLink = "", iframeLink = "") => {
            genericModalContent.innerHTML = "";
            genericModalContent.innerHTML = `
                <div class="modal-header">
                ...
                <div class="modal-body">
                    ...
                    <!-- input-->
                    <div class="form-group">
                        <label for="publishShareLink" i18n="modal.publish.shareLabel">Liens de partage</label>
                        <input value="${shareLink} type="text" class="form-control" id="publishShareLink">
                    </div>
                    <div class="form-group">
                        <label for="publishIframeLink" i18n="modal.publish.iframeLink">Intégration en iframe</label>
                    ...
                    <a class="cardsClose save-close zoomCard" data-bs-dismiss="modal" onclick="mv.changeVersion('${ id }', '${ version }', true)">
                        <i class="ri-save-line"></i>
                        <span i18n="modal.exit.save">Poursuivre les modifications</span>
                    </a>
                    <a class="cardsClose notsave-close zoomCard" data-bs-target="#mod-appversions" data-bs-toggle="modal">
                        <i class="bi bi-x-circle"></i>
                        <span i18n="modal.exit.notsave">Conserver mes modifications et rester sur le projet en cours </span>
        ...
        publish: (id) => {
            if (!id) {
                return alertCustom("Erreur - l'ID n'est pas renseigné. Veuillez contacter un administrateur.", "danger");
            }
            fetch(`${ _config.api }/publish`)
                .then(r => r.json())
                .then(data => {
                    if (!_config?.mviewer_publish) {
                        return alertCustom("Erreur - Configuration manquante pour la publication. Veuillez contacter un administrateur.", "danger");
                    }
                    if (!data.file) {
                        return alertCustom("Erreur - Le lien vers le fichier publié n'est pas accessible. Veuillez contacter un administrateur.", "danger");
                    }
                    return data
                })
                .then(data => {
                    const shareLink = `${_config?.mviewer_publish || ""}${data.file}`;
                    const iframeLink = `
                        <iframe allowFullScreen style="border: none;" height="600" width="800" src="${ shareLink }"></iframe>
                    `;
                    showPublishModal(shareLink, iframeLink)
                    alertCustom("L'application a bien été publiée !", "success");
                })
                .catch(err => {
                    alertCustom("Une erreur s'est produite. Veuillez contacter un administrateur.");
                })
        }
    }
})();
