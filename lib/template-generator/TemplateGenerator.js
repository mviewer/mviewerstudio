...
export default class TemplateGenerator {
  ...
  callTemplate = () => {
    if (this.layer.templateFromGenerator) {
      ...
      let templateFullPath = `${ _conf.conf_path_from_mviewer }${ this.layer.generatorTemplateUrl }`;
      let url = this.layer.templateurl || templateFullPath;
      if (!url) return;
      fetch(_conf.mviewer_instance + url).then(response => response.text()).then(mstContent => {
