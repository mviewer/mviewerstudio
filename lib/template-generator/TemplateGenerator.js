...
export default class TemplateGenerator {
  ...
  addToSelection = ({ detail }) => {
    const selectedIds = this.selected.map(x => x.uuid);
    detail.forEach(id => {
      if (!selectedIds.includes(id)) {
        let componentToAdd = components.filter(c => c.uuid == id)[0];
        componentToAdd.add(generatorComponentsDDArea);
      }
    });
    this.selected = components.filter((c) => detail.includes(c.uuid));

    // selection will be managed by selector component
