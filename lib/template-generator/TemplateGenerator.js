...
export default class TemplateGenerator {
  ...
  dragDropComponentTarget = () => {
    ...
    let colToComplete = cols.filter(
      (c) => c.querySelectorAll(".cardComponent").length < maxItemsByCol
    )[0];
    if (colToComplete) {
    ...
    if (this.selected.length % maxItemsByCol == 1) {
      ...
      generatorComponentsDDArea.insertAdjacentHTML(
        "beforeend",
        `<div class="col-6 colDDArea" id="${uuidCol}"></div>`
      );
  ...
  addToSelection = ({ detail }) => {
    ...
    if (
      this.selectedPanel == "bottom-panel" &&
      this.selected.length == maxCols * maxItemsByCol
    ) {
      alertCustom(
        "Limite atteinte : supprimer un élément du template pour en rajouter !",
        "warning"
      );
      return;
    }
    ...
    let componentsToAdd = new detail.component(
      detail.header,
    ...
    // init tooltips
    [...document.querySelectorAll(".cardComponent .tooltip-info")].forEach(
  ...
  showBadges() {
    ...
    const readableLocationValue = panelLocation.querySelectorAll(
      `option[value='${this.selectedPanel}']`
    )[0].innerHTML;
  ...
  showDragAndDropComponentArea = () => {
    ...
    ddArea.innerHTML = "";
    let ddDomPanel =
      this.selectedPanel == "right-panel" ? RightPanelLocation : BottomPanelLocation;
  ...
  toRightTemplate(isPreview) {
    ...
    const order = [
      ...document
        .getElementById("generatorComponentsDDArea")
        .querySelectorAll(".cardComponent"),
    ].map((x) => x.id);
    // order selected
    const ordered = [];
    // render each
  ...
  toBottomTemplate(isPreview) {
    ...
    cols.forEach((col, index) => {
      let components = [...col.querySelectorAll(".cardComponent")].map(
        (z) => this.selected.filter((s) => s.uuid == z.id)[0]
      );
  ...
  preview() {
    ...
    if (content) {
      ...
      generatorPreviewContent.innerHTML = Mustache.render(
        content,
  ...
  callTemplate = () => {
    if (this.layer.templateFromGenerator) {
      ...
      this.layer.templateurl = this.layer.templateurl || templateFullPath;
      fetch(templateFullPath)
  ...
  readTemplate = (mstContent) => {
    ...
    div.querySelectorAll(".template-component").forEach((el) => {
      ...
      let header = this.componentSelector
        .getHeaders()
        .filter((h) => h.class == component)[0];
      if (header) {
        mv.templateGenerator.addToSelection({
          detail: { component: header.click(), header: header, content: el },
        });
      }
    });
