...
export default class TemplateGenerator {
  ...
  getBasicsMstFile = (content) => {
    return `{{#features}}${content}{{/features}}`
  }

  toTemplate() {
    let componentAsMstString = this.selected.map(component => {
      console.log(component.render());
      return component.render ? component.render() : "";
    })
    return this.getBasicsMstFile(componentAsMstString);
  }
  ...
  preview() {
    ...
    if (content) {
      ...
      generatorPreviewContent.innerHTML = Mustache.render(content, mv.templateGenerator.data);
    }
  }
