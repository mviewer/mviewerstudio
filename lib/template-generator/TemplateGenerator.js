  ...
  saveFile() {
    ...
    fetch(url, {
      ...
      if (r.filepath) {
        ...
        let templateFullPath = `${ _conf.mviewer_instance }${ _conf.conf_path_from_mviewer }${ r.filepath }`
        layer.generatorTemplateUrl = templateFullPath;
        layer.useGeneratorTemplate = true;
      }
    }).catch(e => {
      let layerId = $(".layers-list-item.active").attr("data-layerid");
      let layer = mv.getLayerById(layerId);
      layer.useGeneratorTemplate = false;
    }) 
  }
