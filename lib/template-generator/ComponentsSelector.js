...
const headers = [h1Header(), h3Header(), iframeHeader(), imageHeader(), buttonHeader(), numberHeader(), listHeader(), textHeader()];
...
export default class ComponentsSelector {
  ...
  onValid = () => {
    ...
    mv.templateGenerator.addToSelection({detail: {component: componentSelected, header: this.selected}})
          ...
          <div class="row">
              <div class="col-md-12 mb-2 d-flex justify-content-between align-items-center">
                  <div class="lib-blockComp">
                      ${headers
                        .map((header) => this.componentToHtml(header))
                        .join("")}
                  </div>
              </div>
          </div>`;
