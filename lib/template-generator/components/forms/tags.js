...
class Tags {
  ...
  _configure(config = {}) {
    ...
    const parseBool = (value) =>
      ["true", "false", "1", "0", true, false].includes(value) && !!JSON.parse(value);
    ...
    if (this._searchInput.value) {
      ...
      if (this.getSelectedValues().length) {
        ...
        this._searchInput.size =
          this._config.placeholder.length > 0 ? this._config.placeholder.length : 1;
  ...
  _shouldShow() {
    ...
    if (
      ...
      this.getSelectedValues().length >= this._config.maximumItems
    ) {
      return false;
    }
  ...
  _showSuggestions() {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if (
        !this._config.allowSame &&
        values.indexOf(link.getAttribute(VALUE_ATTRIBUTE)) != -1
      ) {
        hideItem(item);
        continue;
      }
      ...
      const isMatched =
        idx >= 0 || (lookup.length === 0 && this._config.suggestionsThreshold === 0);
  ...
  _positionMenu() {
    ...
    if (vdiff < 0 && bounds.y > h / 2) {
      this._dropElement.style.transform =
        "translateY(calc(-100% - " + this._searchInput.offsetHeight + "px))";
    }
  }
  ...
  addItem(text, value = null, data = {}) {
    ...
    let opts = this._selectElement.querySelectorAll(
  ...
  removeItem(value, noEvents = false) {
    ...
    let opt = this._selectElement.querySelector(
      'option[value="' + escapedValue + '"][selected]'
    );
