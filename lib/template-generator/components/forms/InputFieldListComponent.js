...
export default class InputFieldListComponent {
    ...
    initJsonFields = (jsonFields, subSelection) => {
        ...
        this.subFieldsSelector = new InputTagsComponent(subSelector, subFields, "data-allow-new='true'", subSelection, null, "", "Veuillez choisir un champ type JSON !");
    ...
    getValue = (isPreview) => {
        ...
        let mstSyntax = `{{#${ fieldName }}}<li>${ subSyntax.template || "{{.}}" }</li>{{/${ fieldName }}}`;
        if (isPreview) {
