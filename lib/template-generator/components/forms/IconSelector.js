class IconSelector {
    ...
    eventName = "btnIconClicked-" + this.uuid;
    modalId = "";
    selected = "";
    constructor(label = "Choisir", modalId, previousModalId, defaultIcon) {
        this.label = label;
        ...
        document.addEventListener(this.eventName, () => {
            this.iconSelectorModal.loadModalContent()
        });
        document.addEventListener("btnIconRemove", () => this.remove());

        this.selected = defaultIcon;
    }
    render = () => {
        return `
        <div class="row g-3 align-items-center">
            <div class="form-group" id="formIconGrp-${this.uuid}">
                ...
                <div class="d-flex col-auto">
                    <input  value="${this.defaultIcon || ''}" type="text" class="icon-class-input form-control" placeholder="fas fa-lightbulb" id="iconComponent-${this.uuid}">
                    <span class="selected-icon d-none">${this.selected}</span>
                    <button
