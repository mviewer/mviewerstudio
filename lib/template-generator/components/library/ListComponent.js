...
export const header = () => ({
  ...
  click: () => ListComponent,
  class: "listComponent",
  helpMsg: "Utiliser l'option 'A partir d'un champ JSON' pour un champ de type JSON"
});
...
export default class ListComponent extends TemplateComponent {
  ...
  render = (isPreview) => {
    ...
    if (this.getType() == "static" && isPreview) {
      let tplPreview = this.getFields().map(x => x.processValueToMst(parsableJsonData))[0];
