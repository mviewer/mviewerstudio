...
export const header = () => ({
  icon: "bi bi-123",
  title: mviewer.tr("template.number.title"),
  id: mv.uuidv4(),
...
export default class NumbersComponent extends TemplateComponent {
  ...
  constructor(customHeader, defaultAttrValues) {
    ...
    const textColor = new ColorSelector(
      defaultValues.get("data-bg-color") || "#009688",
      "Couleur du texte"
    );
    const backgroundColor = new ColorSelector(
      defaultValues.get("data-color") || "#f4f4f4",
      "Couleur du fond"
    );
    ...
    const describeInput = new InputTextComponents(
      defaultValues.get("data-text") || "",
      "Description"
    );
    super(
      customHeader || header(),
