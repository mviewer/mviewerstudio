...
export default class ButtonComponent extends TemplateComponent {
  ...
  constructor(customHeader, defaultAttrValues) {
    ...
    const backgroundColor = new ColorSelector(defaultValues.get("data-bg-color") || "#345266", "Couleur du fond");
    const textColor = new ColorSelector(defaultValues.get("data-color") || "#ffffff", "Couleur du texte");
    const textComponent = new InputTextComponents(defaultValues.get("data-text") || "Accéder", "Texte du bouton");
    const iconSelect = new IconSelector("Choisir", "iconsModal", "templageGeneratorModal", defaultValues.get("data-icon"));
    super(
      customHeader || header(),
      defaultTypeFields, [textColor, backgroundColor, textComponent, iconSelect],
      defaultValues
    );
  ...
  template = ({value, text = "visiter", color = "white", backgroundColor = "#345266", icon}) => {
    ...
    >
      <a href="${ value.template }" target="_blank" class="btn" style="background-color: ${ backgroundColor };color:${ color };">
        <i class="${ icon }"></i>
        ${ text }
      </a>
    </div>
