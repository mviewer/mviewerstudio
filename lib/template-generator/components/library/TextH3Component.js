...
export default class TextH3Component extends TemplateComponent {
  ...
  constructor(customHeader, defaultAttrValues) {
    ...
    const textColor = new ColorSelector(defaultValues.get("data-color"), "Couleur du texte");
  ...
  template = ({ value, color = "#009688" }) => {
    ...
    return `
    <div class="${header().class } template-component mb-2" id="${ crypto.randomUUID()}"
    ...
    >
        ...
        Prop : color
        -->
        <h4 style="font-weight:bold; color:${color};">${ mstSyntax }</h4>
    </div>`
  }
}
