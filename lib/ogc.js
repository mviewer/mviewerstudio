...
<csw:GetRecords xmlns:csw="http://www.opengis.net/cat/csw/2.0.2" service="CSW" version="2.0.2" {resultType}>\
    ...
    var _getFields = function (url, typeName, layerid) {
        ...
        .then(data => _DescribeFeatureTypeParse(data, typeName, layerid, url))
    };
    ...
    return {
        ...
        getWfsFeaturesFromWms(url, layerid) {
            _getWFSUrl(url, layerid).then(
                ({describeLayer}) => describeLayer.wfs_url
            ).then(wfs_url => {
                let onSuccess = (data) => {
                    console.log(data)
                }
