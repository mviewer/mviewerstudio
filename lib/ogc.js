var ogc = (function () {
    ...
    var _cswParse = function (xml, csw_url, metadata_url, typeNames) {
        ...
        if (typeNames === "gmd:MD_Metadata") {
            $.each($(xml).find("gmd\\:MD_Metadata, MD_Metadata"), function( index, metadata ) {
                ...
                layer['metadata-csw'] = mv.cleanURL(csw_url) + '?SERVICE=CSW&VERSION=2.0.2&REQUEST=GetRecordById&elementSetName=full&ID&ID=' + layer.identifier;
    ...
    var _wmsAjax = function (url, keyword) {
        var url2 = mv.ajaxURL(mv.cleanURL(url)+'?REQUEST=GetCapabilities&SERVICE=WMS&Version=1.3.0');
        $.ajax({
            type: "GET",
            url: url2,
            crossDomain: true,
    ...
    var _getFields = function (url, typename, layerid) {
        $.ajax({
            type: "GET",
            url: mv.ajaxURL(mv.cleanURL(url)+'?SERVICE=WFS&VERSION=1.1.0&REQUEST=DescribeFeatureType&TYPENAME='+typename),
            crossDomain: true,
            dataType: "xml",
            contentType: "application/xml",
            success: function (data) {
    ...
    var _getWFSUrl = function (wms_url, layerid) {
        $.ajax({
            type: "GET",
            url: mv.ajaxURL(mv.cleanURL(wms_url)+'?SERVICE=WMS&VERSION=1.1.1&REQUEST=DescribeLayer&LAYERS='+layerid),
            crossDomain: true,
            dataType: "xml",
            contentType: "application/xml",
            success: function (data) {
    ...
    return {
        ...
        getFeatures (url, typename, propertyname, option) {
            $.ajax({
                type: "GET",
                url: mv.ajaxURL(mv.cleanURL(url)+'?SERVICE=WFS&VERSION=1.0.0&REQUEST=GETFEATURE&TYPENAME='+typename+'&outputFormat=application/json&propertyName='+propertyname+'&maxFeatures=100'),
                crossDomain: true,
                dataType: "json",
