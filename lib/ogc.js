...
<csw:GetRecords xmlns:csw="http://www.opengis.net/cat/csw/2.0.2" service="CSW" version="2.0.2" {resultType}>\
    ...
    var _wmsCapabilitiesParse = function (data, layerid, themeid) {
        ...
        mv.showStyles(lst, layerid, themeid);
    };
    ...
    var _DescribeFeatureTypeParse = function (data, typeName, layerid, url, themeid) {
        var fields = [];
        ...
        mv.showFields(fields, layerid, themeid);
    };

    var _getFields = function (url, typeName, layerid, themeid) {
        ...
        $.ajax({
            ...
            contentType: "application/xml",
            success: function (data) {
                _DescribeFeatureTypeParse(data, typeName, layerid, url, themeid);
            },
            error: function (xhr, ajaxOptions, thrownError) {
                console.error("WFS DescribeFeatureType request failed", {
                    url: descFeatTypeUrl,
                    xhr: xhr,
                    ajaxOptions: ajaxOptions,
                    thrownError: thrownError
                });
                alert("Echec de la requête WFS DescribeFeatureType :\n" + descFeatTypeUrl);
            }
        });
    };

    var _getStyles = function (url, layerid, themeid) {
        var getCapabilitiesParams = "REQUEST=GetCapabilities&SERVICE=WMS&Version=1.3.0";
        var getCapabilitiesUrl = url.replace(/[?&]$/, '');
        getCapabilitiesUrl = getCapabilitiesUrl.indexOf('?') === -1 ? getCapabilitiesUrl + '?' + getCapabilitiesParams : getCapabilitiesUrl + '&' + getCapabilitiesParams;
        ...
        $.ajax({
            ...
            contentType: "application/xml",
            success: function (data) {
                _wmsCapabilitiesParse(data, layerid, themeid);
            },
            error: function (xhr, ajaxOptions, thrownError) {
                console.error("WMS GetCapabilities request failed", {
                    url: getCapabilitiesUrl,
    ...
    var _describeLayerParse = function (xml, layerid, themeid) {
        var wfs_url = $(xml).find("LayerDescription").attr("wfs").split("?")[0];
        var typeName = $(xml).find("LayerDescription Query").attr("typeName");
        _getFields(wfs_url, typeName, layerid, themeid);

    };
    var _getWFSUrl = function (wmsUrl, layerid, themeid) {
        ...
        $.ajax({
            ...
            contentType: "application/xml",
            success: function (data) {
                _describeLayerParse(data, layerid, themeid);
            },
            error: function (xhr, ajaxOptions, thrownError) {
                console.error("WMS DescribeLayer request failed", {
                    url: descLayerUrl,
                    xhr: xhr,
                    ajaxOptions: ajaxOptions,
                    thrownError: thrownError
                });
                alert("Echec de la requête WMS DescribeLayer :\n" + descLayerUrl);
            }
        });
    };
    ...
    return {
        ...
        getFieldsFromWMS (url, layerid, themeid) {
            _getWFSUrl (url, layerid, themeid);
        },

        getStylesFromWMS (url, layerid, themeid) {
            _getStyles (url, layerid, themeid);
        },
