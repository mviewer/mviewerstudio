...
/**
 * This class will replace old iconPicker
 * This class will read metadata Font-Awesome file to
 * create icons selector without manual list.
 * Need a modal to work. 
 */
class iconPickerComponent {
	...
	constructor(modalId, previousModal, metaFile) {
		...
		fetch(metaFile  || "lib/Font-Awesome/icons.json").then(r => r.json()).then(allIcons =>
			this.iconsMetadata = allIcons
		);
	...
	content = (v) => {
		...
		if (v) {
			iconsSet = iconsSet.filter(name => name.toLocaleLowerCase().includes(v.toLocaleLowerCase()))
		}
	...
	loadModalContent = (v) => {
		...
		this.contentDiv.innerHTML = `
			...
			<div class="modal-footer">
				${
					this.previousModal ?
					`<button type="button" class="btn btn-primary" data-bs-target="#${ this.previousModal }" data-bs-toggle="modal">Annuler</button>`
					:
