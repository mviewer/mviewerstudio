...
function switchModule() {
  ["btnNext", "btnPrev"].forEach((buttonClassName) => {
    ...
    let btnEl = document.querySelectorAll(
      `#stepStudioContent .tab-pane:not([style*='display: none']) .${buttonClassName}`
    );
    ...
    btnEl.forEach(function (item, index) {
      item.addEventListener("click", function () {
        ...
        let tabElement = document.querySelectorAll(
          "#stepStudio li:not([style*='display: none']) a"
        )[id];
...
function displayWizard(x) {
  ...
  let btnWiz = document.querySelectorAll(
    "#stepStudioContent .btnNext,#stepStudioContent .btnPrev"
  );
