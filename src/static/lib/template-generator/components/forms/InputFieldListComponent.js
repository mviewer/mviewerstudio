...
export default class InputFieldListComponent {
  ...
  initJsonFields = (jsonFields, subSelection) => {
    ...
    let jsonFieldDomEl = document
      .getElementById(this.uuid)
      .parentNode.querySelector(".jsonFields");
    if (!jsonFieldDomEl) return;
    let subSelector = "sub-select-" + this.uuid;
    this.subFieldsSelector = new InputTagsComponent(
      subSelector,
      subFields,
      "data-allow-new='true'",
      subSelection,
      null,
      "",
      mviewer.tr("template.select_json_field")
    );
  ...
  onSelect = ({ value }) => {
    ...
    // clean
    let jsonField = document.getElementById(this.divId).querySelector(".jsonFields");
    if (jsonField) {
      jsonField.innerHTML = "";
    }
  ...
  render = (value) => {
    return `<select onchange="mv.createDispatchEvent('${
      this.eventName
    }', this)"class="form-control form-control-sm li-field-name component-form" id="${
      this.uuid
    }">
