...
def _qgs_project_payload(qgs_dir: str, absolute_file: str) -> dict:
    ...
    return {
        ...
        "path": relative_file,
        "lastModified": datetime.fromtimestamp(path.getmtime(absolute_file)).isoformat(),
    }


def _extract_qgs_zip(uploaded_file, qgs_dir: str, filename: str) -> tuple[str, list[dict]]:
    ...
    try:
        with ZipFile(uploaded_file.stream) as archive:
            ...
            for member in file_members:
                ...
                if path.commonpath([path.abspath(destination_dir), destination]) != path.abspath(
                    destination_dir
                ):
